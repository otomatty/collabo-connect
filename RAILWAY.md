...
3. **テストデータを投入**（認証ユーザー → profiles → 投稿 → 参加情報 → AI 回答の順で投入）:
   ```bash
   cd api && railway run -- node scripts/run-seed.mjs
   ```

- 事前に [Railway CLI のインストール](https://docs.railway.com/develop/cli) と `railway login` が必要。
- スキーマは冪等なので、再実行しても既存テーブルはそのままになる。
- シードも冪等です。同じコマンドを再実行すると、固定 ID のテストユーザーと関連データを更新します。
- シード対象の認証ユーザー email は `taro.tanaka@example.com` などの固定値です。マジックリンクログインの検証にも使えます。
