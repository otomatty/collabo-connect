...
/**
 * Generate (or no-op) today's daily question. Shared by the cron HTTP endpoint
 * and the Worker `scheduled()` cron trigger.
 */
export async function runGenerateDailyQuestion(
  db: DbClient,
  apiKey: string | undefined
): Promise<GenerateDailyQuestionResult> {
  const today = getTodayInJst();

  const existing = await db.query(
    "SELECT id FROM ai_questions WHERE date = $1 LIMIT 1",
    [today]
  );
  if (existing.rows.length > 0) {
    return { message: "Today's question already exists", date: today, alreadyExists: true };
  }

  const recent = await db.query<{ question: string }>(
    "SELECT question FROM ai_questions ORDER BY date DESC LIMIT 15"
  );
  ...
  try {
    ...
    parsed = await generateQuestionWithGemini(today, recentList, apiKey);
  } catch (err) {
    source = "fallback";
...
router.post("/generate-daily-question", requireCronSecret, async (c) => {
  try {
    const db = c.get("db");
    const result = await runGenerateDailyQuestion(db, c.env.GEMINI_API_KEY);
